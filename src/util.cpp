...
const char * const BITCOIN_CONF_FILENAME = "etp.conf";
const char * const BITCOIN_PID_FILENAME = "etpd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "etp" is a composite category enabling all Etp-related debug output
            if(ptrCategory->count(string("etp"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EtpCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EtpCore
    // Mac: ~/Library/Application Support/EtpCore
    // Unix: ~/.etpcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EtpCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EtpCore";
#else
    // Unix
    return pathRet / ".etpcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty etp.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override etp.conf
