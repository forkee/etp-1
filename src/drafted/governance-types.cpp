...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "EtpNetwork") type = EtpNetwork;
    if(strType == "EtpNetworkVariable") type = EtpNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == EtpNetwork) s = "EtpNetwork";
    if(type == EtpNetworkVariable) s = "EtpNetworkVariable";
